...
- That same probe reads which transcription model the server has resident (`GET /api/v1/health`) and sends its name, because the server rejects a request with no `model` field outright ("Missing 'model' field in request") — there is no way to say "whatever you have loaded". Naming a model it doesn't have loaded is not an error either, it is a stall: the server loads that model while the first segment waits. Only an entry that is `loaded` *and* of type `transcription` counts — the top-level `model_loaded` is just the last model touched, an LLM as often as not. Among those, an unquantized model is preferred over a quantized one (matched on a `Q`/`q` followed by a digit, in the id or the checkpoint): quantization is a concession to the machine running the model, and the remote box is the one place here where it need not be made.

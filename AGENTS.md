...
- On quit or reload the config hands the system's default input back to a non-Bluetooth device — macOS keeps a headset in its call profile (HFP) for exactly as long as it is the *default input*, and the `mic` pin governs only lw-record, not Teams, Zoom, or the Sound settings pane, which holds the input open the whole time that pane is on screen. Measured here: the headset returned to A2DP (2ch/44100 from 1ch/16000) the instant the default input moved to the built-in mic, while another process still had a microphone open — so the release is about which device is selected, not about closing streams. `hs.shutdownCallback` stops the wake listener first, then moves the default input; it fires for reload as well as quit and must stay synchronous.
...
- The remote endpoint is whisper.cpp's own `examples/server`, not an OpenAI-compatible wrapper — it was lemonade before, which turned out to just be embedding llama.cpp for other tasks that were already served by a separate llama instance here, so lemonade was dropped entirely. There is no `model` field on the request and no model list on the health check: the server always transcribes with whatever it was launched with (or last handed to `/load`), so the probe in `checkApiAvailable` only has to prove the server is reachable, nothing more.

- The overlay's pin locks a dictation hands-free: while it is lit, trigger release, the voice silence timeout and the voice cap are all ignored, and clicking the pin again is the release. The lock lives only for one recording — every stop path clears it — so a forgotten lock can never carry into the next dictation.
- The pin's icons are SF Symbols (`pin`, `pin.fill`) baked to PNG by `tools/lw-pin-icons.swift` — `hs.image` cannot load SF Symbols by name, and the emoji it replaced ignores `textColor` alpha, so locked and unlocked looked identical.
- Clicking the overlay makes Hammerspoon the frontmost app at mouseUp, even with `clickActivating(false)` (measured with synthetic clicks). Left alone, that moved the insert target onto Hammerspoon and the text went to the clipboard. The focused window and target are taken at mouseDown, focus is handed back after mouseUp, and a stop from the pin uses the mouseDown target.
- Pasting into Royal TSX (dictation and the Recent Dictations menu alike, via `pasteText`) posts Cmd down, V down, V up, Cmd up as four separate events after a 0.8 s wait (0.4 s was too short and still pasted stale text at times), not `hs.eventtap.keyStroke` — Royal TSX strips a modifier that arrives only as a flag on the V (Cmd+V and Ctrl+V both typed a bare "v"), and it copies the Mac clipboard into the remote session asynchronously, so an immediate paste delivers the previous dictation. The wait is blocking on purpose: the Enter-mode keystroke fires 0.15 s after insertion and must not overtake the text.
- The pin tracks mouseUp itself — otherwise the release falls through to the background's older click-to-keep-open handler, and one click toggles both.
